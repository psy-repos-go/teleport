...
type CLIConf struct {
	...
	// LocalProxyTLS specifies whether local proxy will open auth'd tunnel.
	LocalProxyTLS bool
...
func Run(args []string, opts ...cliOption) error {
	...
	proxyDB.Flag("tls", "Open authenticated tunnel so clients don't need to authenticate").BoolVar(&cf.LocalProxyTLS)
